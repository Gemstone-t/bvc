...
function adjustStyles(){
    ...
    if(screenWidth > 1024){
        ...
        var e_str = [
            ...
            {'name': 'body .nav-tool-bar .nav-home .menu ul li a', 'fontSize': 17, 'lineHeight': 25},
        ];
        ...
        var e_str = ['h1', 'h2', 'h2.alt', 'h3', 'h3.alt', 'h4', 'h5', 'a.nav-link', 'a.utility-nav', 'p.eyebrow', 'button', '.button', 'p.XXL', 'p.XL', 'p.L', 'p.M', 'a', 'p.S', 'p.XS', 'p.call-out', 'a.text-link', 'select', 'label', 'input', 'body .nav-tool-bar .nav-home .menu ul li a'];
        var element = $('h1');
        for(var i = 0; i < e_str.length; i++){
            var element = $(e_str[i]);
            for (var j = 0; j < element.length; j++) {
